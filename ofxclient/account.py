...
class Account(object):
    ...
    def download(self, days=60, end_date=None):
        ...
        as_of = time.strftime("%Y%m%d", start_date.timetuple())
    ...
    def download_parsed(self, days=60, end_date=None):
        ...
        if IS_PYTHON_2:
            return OfxParser.parse(
                self.download(days=days, end_date=end_date)
            )
        else:
            return OfxParser.parse(
    ...
    def statement(self, days=60, end_date=None):
    ...
    def transactions(self, days=60, end_date=None):
